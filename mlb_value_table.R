...
if(file.exists(paste0("H:/My Drive/R_playground/", format(mdy(date), "%m"), "-", format(mdy(date), "%Y")))){
  cat("already exists")
} else {
  dir.create(paste0("H:/My Drive/R_playground/", format(mdy(date), "%m"), "-", format(mdy(date), "%Y")))
}
...
#betplusev <- read_html("https://betplusev.com/mlb/daily-projections/batters")

#bettingpros <- read_html("https://www.bettingpros.com/mlb/picks/prop-bets/")
#bp_table <- html_nodes(bettingpros, "table") %>% html_table(fill = TRUE)

# Get the HTML of the website
#html <- read_html("https://www.dimers.com/best-props/mlb")

# Get the XML path
#xml_path <- html %>%
#  xpath("/html/body/app-root/main/app-best-props/div[1]/app-best-props-block/div/div[3]/div")
...
user <- "bobbydoll11@gmail.com"
pass <- "Greathat24147!"

client_id <- "maWqPuUi-nHEqHUqI8X2kE8DU_GXAciIxbtM7sNIJdo"
client_secret <- "7P7j8JhI1Qb3oV-h4BZqDx3GKHWsa501tlnrIowmvBU"
